...
class AppleArticle extends StatefulWidget {
  AppleArticle({Key key}) : super(key: key);

  @override
  _AppleArticleState createState() => _AppleArticleState();
}

class _AppleArticleState extends State<AppleArticle> {
  @override
  initState() {
    super.initState();
    final article = Provider.of<News24CategoryProvider>(context, listen: false);
    article.getAppleArticle();
    print(article);
    inspect(article);
  }
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: RefreshIndicator(
        onRefresh: () async => await articleProvider.getAppleArticle(),
